...
export const fetchTasks = createAsyncThunk<Task[]>(
  "tasks/fetchTasks",
  async () => {
    const response: AxiosResponse<TaskResponse | null> =
      await axiosAPI.get("/tasks.json");
...
export const addTask = createAsyncThunk<Task, string>(
  "tasks/addTask",
  async (title) => {
    const response: AxiosResponse<{ name: string }> = await axiosAPI.post(
      "/tasks.json",
...
export const updateTaskStatus = createAsyncThunk<Task, Task>(
  ...
  async (task) => {
    const response: AxiosResponse<Task> = await axiosAPI.put(
      `/tasks/${task.id}.json`,
      task,
    );
